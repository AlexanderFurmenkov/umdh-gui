...
UmdhWidget::UmdhWidget(gui::ISettings *pSettings,
                       gui::IUmdh *pUmdh,
                       gui::ICommandExecuter *pCommandExecuter,
                       QWidget *parent) :
    QWidget(parent),
    m_pSettings(pSettings),
    m_pUmdh(pUmdh),
    m_pCommandExecuter(pCommandExecuter),
    m_pUi(new Ui::UmdhWidget)
{
    ...
    m_pUi->pathToGflagsLineEdit->setText(QString::fromStdString("C:\\Program Files (x86)\\Windows Kits\\10\\Debuggers\\x64\\gflags.exe"));
...
void UmdhWidget::updateGuiState(bool isUmdhValid, bool isTargetProcessRunning, bool isMultipleProcessesRunning)
{
    ...
    m_pUi->gflagsGroupBox->setEnabled(!m_pSettings->getTargetProgramm().empty());
}
...
void UmdhWidget::on_gflagsBeforePushButton_clicked()
{
    m_pCommandExecuter->execute(m_pUi->pathToGflagsLineEdit->text().toStdWString(), L"-i " + m_pSettings->getTargetProgramm() + L" +ust");
}

void UmdhWidget::on_gflagsAfterPushButton_clicked()
{
    m_pCommandExecuter->execute(m_pUi->pathToGflagsLineEdit->text().toStdWString(), L"-i " +  m_pSettings->getTargetProgramm() + L" -ust");
}
